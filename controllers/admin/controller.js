 ...
 */
    forgotPassword:async(req,res)=>{
     try {
        ...
        let update = await service.updateUser({_id:admin._id},{$set:{emailOtp:otp,emailOtpExpireTime:Date.now()}})
        let data={
            email:admin.email,
            otp:otp
        }
        await mailFunctions.forgotPassword(data)
        return response(res, statusCode.data.SUCCESS, update, messages.SuccessMessage.FORGET_SUCCESS)
     } catch (error) {
        console.log("============>error",error)
        return response(res, statusCode.data.SOMETHING_WRONG, error, messages.ErrorMessage.SOMETHING_WRONG)
     }
